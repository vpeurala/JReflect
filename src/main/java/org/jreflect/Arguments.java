...
class Arguments {
	 ...
	 * 
	 * # byte to short, int, long, float, or double # short to int, long, float,
	 * or double # char to int, long, float, or double # int to long, float, or
	...
	boolean matchesMethod(Method m) {
		...
		for (int i = 0; i < methodParameterTypes.size(); i++) {
			...
			Option<Class<?>> argumentClass = argumentClasses.get(i);
	...
	private boolean isExactOrCoercablePrimitiveVersionOf(
			Class<?> possiblePrimitiveType, Class<?> possibleWrapperType) {
		if (!possiblePrimitiveType.isPrimitive()) {
			return false;
		}
		List<Class<?>> coercions = coercionRules.get(possibleWrapperType);
	...
	public String toString() {
		...
		for (int i = 0; i < argumentClasses.size(); i++) {
			...
			Option<Class<?>> argumentClassOption = argumentClasses.get(i);
